...
import 'information_tab/cover_image_section.dart';
import 'information_tab/product_metadata_fields.dart';
import 'information_tab/artist_section.dart';
import 'information_tab/product_identity_fields.dart';
import 'information_tab/rights_fields.dart';
import 'information_tab/validation_utils.dart';
import 'information_tab/data_persistence.dart';
...
class InformationTab extends StatefulWidget {
  ...
  final bool isMobile;
  final void Function(List<Map<String, dynamic>> artistData) onSave;
...
class InformationTabState extends State<InformationTab> {
  ...
  void _initializeState() {
    ...
    _selectedMetadataLanguage = widget.selectedMetadataLanguage ?? (widget.metadataLanguages.isNotEmpty ? widget.metadataLanguages[0] : null);
  ...
  Future<void> _fetchAllArtists() async {
    try {
      List<Map<String, dynamic>> allArtists =
          await ApiService().fetchAllArtistsWithIds();
      setState(() {
        artistSuggestions = List<String>.from(
          allArtists.map((artist) => artist['name'] as String),
        );
      });
    } catch (e) {
  ...
  Future<void> _fetchLabels() async {
    try {
      final labels = await ApiService().fetchLabels();
  ...
  bool _validateFields() {
    return _validationUtils.validateAllFields(
      ...
      language: _selectedMetadataLanguage?.code, // Fix: pass code string
      genre: _selectedGenre,
      subgenre: _selectedSubgenre,
      price: _selectedPrice,
    );
  ...
  void _onProductLoaded(Product loadedProduct) {
    setState(() {
      ...
      _selectedMetadataLanguage = loadedProduct.metadataLanguage;
  ...
  void dispose() {
    ...
    widget.releaseTitleController.removeListener(_onFieldChanged);
    widget.releaseVersionController.removeListener(_onFieldChanged);
    widget.primaryArtistsController.removeListener(_onFieldChanged);
    widget.upcController.removeListener(_onFieldChanged);
    widget.labelController.removeListener(_onFieldChanged);
    widget.cLineController.removeListener(_onFieldChanged);
    widget.pLineController.removeListener(_onFieldChanged);
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Build the main layout with proper components
    return Stack(
      children: [
        _isLoadingProduct
