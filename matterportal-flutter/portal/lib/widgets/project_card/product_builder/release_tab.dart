...
import 'package:portal/screens/home/project_view/distribution_success.dart';
...
class _ReleaseTabState extends State<ReleaseTab> {
  ...
  String? _validateProduct(Product product) {
    ...
    if (product.primaryArtistIds.isEmpty) {
    ...
    if (product.metadataLanguage!.code.isEmpty) {
  ...
  String? _validateTrack(Track track, int index) {
    ...
    if (track.primaryArtistIds!.isEmpty) {
  ...
  Future<void> _loadProductState() async {
    try {
      ...
      if (productData != null && mounted) {
        setState(() {
          _currentState = productData.state;

          // Load release date and time

          selectedDate = DateTime.parse(productData.releaseTime);
          ...
          useRollingRelease = productData.useRollingRelease;
  ...
  Future<void> _distributeProduct() async {
    ...
    try {
      ...
      await ApiService().distributeProduct(
        user.uid,
        widget.projectId,
        widget.productId,
        selectedStores,
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow:
                          _canDistribute()
                              ? [
                                BoxShadow(
                                  color: const Color(0xFF9D6BFF).withAlpha(200),
                                  spreadRadius: 0,
                                  blurRadius: 12,
