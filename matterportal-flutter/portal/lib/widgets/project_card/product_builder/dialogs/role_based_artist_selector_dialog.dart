  ...
  Future<void> _loadArtists() async {
    ...
    try {
      if (widget.collection == 'songwriters') {
        _filteredArtists = await ApiService().fetchAllSongwriters();
      } else {
        _filteredArtists = await ApiService().fetchAllArtists();
      }
    } finally {
          ...
          children: [
            ...
            Expanded(
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Expanded(child: _buildSelectedArtistsList()),
                  const VerticalDivider(color: Colors.grey),
                  Expanded(child: _buildAvailableArtistsList()),
                ],
              ),
            ),
            const SizedBox(height: 24),
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                leading: FutureBuilder<String?>(
                  future: ApiService().getArtistProfileImage(artist['name']),
                  builder:
              ...
              _isLoading
                  ...
                  : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        leading: FutureBuilder<String?>(
                          future: ApiService().getArtistProfileImage(artist),
                          builder:
                              (context, snapshot) => CircleAvatar(
                                backgroundImage:
                                    snapshot.data != null
              ...
              onChanged: (value) {
                setState(() {
                  final filteredRoles = widget.availableRoles
                      ...
                      .toList();
                  tempSelected = Set.from(filteredRoles.where((role) => tempSelected.contains(role)));
                });
              },
            ),
            const SizedBox(height: 8),
